...
let createUser = async function (req, res) {
    try {
        ...
        if (!isValid(email)) {
            return res.status(400).send({ status: false, msg: "Please provide a Email d or a Valid Email Id" });
        }
        ...
        if(!isValid(address.city)){
            return res.status(400).send({ status: false, msg: "City should be Present or City should be Valid" });
        }

        if(!validCity.test(address.city)){
            return res.status(400).send({ status: false, msg: "City cannot be Number" });
        }
        ...
        if(!validPincode.test(address.pincode)){
            return res.status(400).send({ status: false, msg: "PinCode cannot be Alphabetic" });
        ...
        let createUser= await usermodel.create(requestBody)
            return res.status(201).send({status:true, msg: "User Created" ,data: createUser }) //sending data in response
